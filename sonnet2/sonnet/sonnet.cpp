...
bool find_phonetic_ending(const char word[], char ending[]) {
  ...
  while (!in.eof()) {
    ...
    if (!strcmp(word, dict_word)) {
      cout << "found the word it is " << dict_word << endl;


      in >> syllable;
      cout << syllable << endl;

      while (strcmp(syllable, "\n")) {
        cout << "hewre" << endl;
        if (!concatentating) {
          for (int i = 0 ; syllable[i] != '\0' ; i ++) {
            char letter = syllable[i];
            if (letter == 'A' || letter == 'E' || letter == 'I' || letter == 'O' || letter == 'U') {
              concatentating = true;
              strcat(ending, syllable);
            }
          }
